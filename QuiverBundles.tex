...
\item to enlarge some existing connections between quiver bundles and Higgs bundles, building on the relationship where the former, at least in Dynkin type $A$, appear as fixed points of an algebraic $\mathbb C^\times$-action on the moduli space of the latter (cf. for example \cite{R18});
...
investigations.
%\textit{Porta-Sala:}
While our own motivation comes from Higgs bundles, an origin point for this programme is King's treatment of moduli of representations of algebras in \cite{FA94}.  In the bundle-theoretic context where our interests reside, moduli stacks of quiver bundles have been constructed in great generality in
\cite[Proposition 7.1 and Corollary 7.2]{HP25} in the context of derived stacks
and derived analytic stacks. Nevertheless, in this paper, we provide an alternate, perhaps simpler, construction
which we can work with in the context of $1$--stacks over the fppf site of affine schemes
over a ground ring $k$, without using any derived technology. Hence, proofs in the
non-derived setting are perhaps simpler in our approach. At the same time, some of
our results, in the non-derived setting, are slightly more specific ---
see remarks \ref{rmk:Porta-Sala-alg-1} and \ref{rmk:Porta-Sala-alg-Higgs}. At the same time, we recognize parallel results in the pursuit of moduli stacks of labelled diagrams, including \cite{QT12}, which employs moduli of quiver-bundle like data (specifically, quivers of sections in the sense of \cite{CG08}) and obtains a stackified linear series construction for toric Deligne-Mumford stacks, building on a characterization in \cite{TS08}.

To accomplish our goals in as ``model-agnostic'' a way
...
\begin{rmk}\label{rmk:Porta-Sala-alg-1}
%\textit{Porta-Sala:}
We should clarify here that such algebraicity results are available in far greater generality.
For instance, \cite[Proposition II.2.8]{DPS25} along with To\"en-Vaqui\'e's representability theorem
(\cite[Theorem II.2.22]{DPS25} or \cite[Theorem 3.6]{TV07}) show that the derived moduli stack of quiver
representations in perfect complexes over a smooth variety over a field $k$ is locally geometric and locally
of finite presentation. While our algebraicity results can be recovered from this, our constructions
and method of proof are different. Furthermore, our methods apply directly in the non-derived setting
and yield additional properties of the constructed moduli stacks
under suitable hypotheses on the base stack --- see \cref{thm:algebraicity} and
\cref{rmk:more-relaxed-hypotheses}.
\end{rmk}
In \cref{sec:QuiverVarieties}, we sketch how to recover quiver varieties and
...
%\textit{JAlg Referee Report, point 2:}
We will then consider some assumptions on
$\St_{\s{C}}^n$ for all $n$, modeled on the properties of the categories
of quasicoherent sheaves and vector bundles over $1$--stacks in (non-derived) algebraic geometry
--- that is, objects of $\St^1_{\CRing^{\heartsuit, \op}_{/k}, fppf}$.
...
the unstraightening of $\Vect$ over $U \times X$.
\end{defn}
...
\begin{rmk}
%\textit{JAlg Referee Report, point 1:}
In the concrete geometric contexts, this makes $\mc{M}_{\Vect(X)}$ the moduli stack of vector bundles
over $X$.
In particular, for $\St_{\CRing_{/k}^{\heartsuit, \op}}^1$, $\mc{M}_{\Vect(X)}$ is precisely
the moduli stack of vector bundles over $X$ (as defined in, for example,
\cite[First paragraph of \S 7.3]{CW17}). To see this, it suffices to unwrap the definition
of the Grothendieck construction of $\Vect$. A similar argument shows that it coincides with
the moduli stack of vector bundles in the derived setting, as defined in, for example \cite[\S 3]{AI22}.
\end{rmk}
...
This is the case we will be most interested in for the present work.

\begin{rmk}
%\textit{JAlg Referee Report, point 2:}
We should clarify that these assumptions are satisfied by all $1$--stacks irrespective of any form
of algebraicity (Artin, Deligne-Mumford, etc.) or other special properties.
\end{rmk}

\begin{rmk}
%\textit{JAlg Referee Report, point 3:}
For $n = \infty$, if we take $\s{C}$ to be the opposite of animated commutative rings,
$\St_\s{C}$ to be derived stacks, $\QCoh$ and $\Vect$ to be the functors sending derived
stacks to the quasicategories of complexes of the respective objects,
and $\mc{V}_X := \underline{\Spec}_X\mathrm{L}\Sym^*(-)$,
then these assumptions should be satisfied (again, regardless of algebraicity/geometricity).
We wish to explore the applicability of the techniques of this paper in the derived setting in future work.
It is possible that this will yield slightly more specific results, even in the derived setting,
than the ones available in the literature
(see \cref{rmk:Porta-Sala-alg-1} and \cref{rmk:Porta-Sala-alg-Higgs}).
\end{rmk}
...
\begin{rmk}
%\textit{JAlg Referee Report, point 3:}
Since the various moduli spaces related by non-Abelian Hodge theory have analytic
forms, and the complex non-Abelian Hodge correspondence is heavily dependent on PDE theory,
it might be worthwhile to explore the applicability of the techniques of this
paper in the context of analytic stacks. Since the above assumptions hold
for the $(2, 1)$-category of stacks with no additional constraints like algebraicity,
by formal/categorical reasons,
we expect them to hold for the analytic stacks of \cite{CS19} and \cite{BKK24} as well,
for similar reasons, as these are just stacks over sites of analytic and simplicial commutative complete bornological rings respectively.
\end{rmk}
...
\begin{rmk}
%\textit{JAlg Referee Report, point 4:}
We discuss the connection of the present work with ordinary and Nakajima quiver varieties in
\cref{sec:QuiverVarieties}.
\end{rmk}
...
\begin{rmk}
%\textit{JAlg Referee Report, point 5:}
Labels on vertices of a quiver are used to construct the space of representations
as a finite dimensional vector space which is then used to construct quiver varieties.
In our case, the space of representations is replaced by a moduli stack $\mc{M}_{\Vect(X), I}$
parametrizing diagrams of vector bundles of shape $I$ for a quiver $I$.
Taking $X$ to be the point gives a moduli stack of representations in vector spaces,
but of all possible dimensions. Then,
if a labeling is given for $I$, we can consider the full substack of $\mc{M}_{\Vect(X), I}$
consisting of those diagrams whose vertices have ranks/dimensions corresponding to the labels.
It is likely that these substacks have similar properties to the original moduli stack
and under a suitable notion of GIT for stacks (for example, \cite{AHLH24}), they recover
classical quiver varieties --- see \cref{sec:QuiverVarieties}.
However, the main results of this paper are not concerned with such substacks or quiver varieities
and we will avoid discussing labels any further.
\end{rmk}
...
\begin{rmk}
%\textit{Porta-Sala:}
This result is likely a categorical truncation of the $2$--Segality established in \cite[Lemma II.4.1]{DPS25}
\end{rmk}
...
\cup \set{\mc{M}_{1, (i, j)} \to[t] \mc{M}_{0, j}}_{i, j \in [n]}
\]
...
\begin{rmk}\label{rmk:more-relaxed-hypotheses}
For algebraicity, the hypotheses of \cref{thm:algebraicity} are
not the most relaxed possible: see \cref{rmk:Porta-Sala-alg-1}.
However, the more constrained hypotheses allowed us to prove some additional properties
of the diagonal of the constructed moduli stacks of quiver bundles that are not
immediate from the results available in the literature on quiver bundles.
Furthermore, it might be possible to use the same techniques of this paper to obtain
similar or stronger results with more relaxed hypotheses. For instance,
...
quiver varieties and Nakajima quiver varieties from our framework and we now provide
...
the group action used in quiver theory to construct quiver varieties to a group action
...
a disjoint union of quiver varieties for each choice of dimension vector.
...
\begin{thm}
A $U$--point of $\mc{M}_{\mathrm{Higgs}(X, K), 1}$
...
holds, the stacks
$\mc{M}_{\mathrm{Higgs}(X, K)}$ and $\mc{M}_{\mathrm{Higgs}(X, K), 1}$ are Artin, locally of finite
presentation, quasi-separated and has affine stabilizers. If $X$ and $\mc{M}_{\Vect(X)}$ have affine diagonal,
then so do $\mc{M}_{\mathrm{Higgs}(X, K)}$ and $\mc{M}_{\mathrm{Higgs}(X, K), 1}$.
\end{cor}
\begin{proof}
It suffices to see that they are constructed as iterated pullbacks of stacks satisfying these properties.
\end{proof}

\begin{rmk}\label{rmk:Porta-Sala-alg-Higgs}
%\textit{Porta-Sala:}
This result also has a derived counterpart. \cite[Lemma 5.3.1]{PS20} combined with
\cite[Corollary 4.6]{PS23} yield algebraicity and local finite presentation of the derived moduli
stacks of Higgs bundle diagrams over a very general base stack. The algebraicity and local finite
presentation of the non-derived stacks can be recovered from this.
However, our methods of construction and proof are completely in the non-derived setting and yield
some more properties of the constructed moduli stacks.
\end{rmk}
...
\cites{Por17, PS20} but originating in works of Carlos Simpson --- and
...
$X_{\mathrm{dR}}$ is the de Rham shape of $X$, described in \cites{Por17, PS20}.
